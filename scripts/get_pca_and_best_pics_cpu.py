...
pca = IncrementalPCA(n_components=num_neurons, batch_size = 25*16*16)
for i, sample in tqdm(enumerate(dataset)):
    acts = sample["diff.pth"].permute((0, 1, 3, 4, 2))
    acts_flat = acts.reshape(-1, acts.shape[-1])
    # take random 10%
    acts_subset = acts_flat[torch.randperm(acts_flat.shape[0])[:int(acts_flat.shape[0] * 0.2)]]
    pca.partial_fit(acts_subset)
    if i == cutoff - 1:
        break
os.makedirs(f"resourses/pca/{block_name}", exist_ok=True)
np.save(f'resourses/pca/{block_name}/pca_components.npy', pca.components_)
#then transform can be used using         X_transformed = X @ self.components_.T    

for i, sample in tqdm(enumerate(dataset)):
    diff = sample["diff.pth"].permute((0, 1, 3, 4, 2))
    diff_flat = diff.reshape(-1, diff.shape[-1])
    acts = pca.transform(diff_flat)
    acts = acts.reshape(diff.shape[0], diff.shape[1], diff.shape[2], diff.shape[3], acts.shape[1])
...
torch.save(topk_values, f'resourses/pca/{block_name}/topk_vals.pt')
torch.save(topk_indices, f'resourses/pca/{block_name}/topk_indices.pt')
torch.save(topk_heatmaps, f'resourses/pca/{block_name}/topk_heatmaps.pt')

mean, std, act_rate = running_stats.get_mean_std_act_rate()
torch.save(mean, f'resourses/pca/{block_name}/mean.pt')
torch.save(std, f'resourses/pca/{block_name}/std.pt')
torch.save(act_rate, f'resourses/pca/{block_name}/act_rate.pt')
...
if compute_previews:
    ...
    def process_feat_idx(feat_idx):
        os.makedirs(f"resourses/pca/{block_name}/imgs/{feat_idx:06d}", exist_ok=True)
        for i in range(topk_indices.shape[0]):
            # try:
                img = Image.fromarray(images[topk_indices[i, feat_idx].item()])
                img = img.resize((128, 128))  # Resize the image to 128x128
                img.save(f"resourses/pca/{block_name}/imgs/{feat_idx:06d}/{i:06d}.png")
